<!DOCTYPE html>
<html>
<head>
	<title>{{ $title }}</title>
    <style>
    .logo{
        width:50;
    }
    .title{
        text-align:left;
    }
    .diteruskan,.tr,.td{
        border: 1px solid #000;
        border-collapse: collapse;
    }                                                                         
        body{
            font-size:14px;
        }
    </style>
</head>
<body>
	<table style="width:100%">
        <tr>
           
                 {{-- <td rowspan="3" style="width:60px"><img class="logo" src="{{asset('image/setting/'.$setting->logo)}}"></td> --}}
          
                 <td rowspan="3" style="width:60px"><img class="logo" src="{{public_path('image/setting/'.$setting->logo)}}"></td>
           
            <td colspan="7" class="title" style="font-size:17px;vertical-align: bottom;">RUMAH SAKIT</td>        
        </tr>
        <tr>
            <td class="title"colspan="7" style="font-size:17px;vertical-align: top;"><strong><u>PKU MUHAMMADIYAH SEKAPUK</u></strong></td>        
        </tr>
        <tr>
            <td class="title"colspan="5" style="font-size:10px;vertical-align: top;"><i>CREATIVE, ACTIVE, RESPONSIBILITY, EMPATY(CARE)</i></td>        
        </tr>
        <tr>
            <td colspan="8" style="vertical-align: top;"> <hr></td>
        </tr>
        <tr>
            <td colspan="8" style="text-align:center"><strong> <u>LEMBAR DISPOSISI</u></strong></td>
        </tr>
        <tr>
            <td colspan="7" style="text-align:left;height:20px"></td>
            
        </tr>
        @foreach ($surat as $item)
        <tr>
            <td colspan="3" style="text-align:left;">Tanggal Penerimaan Surat</td>
            <td style="text-align:center;width:5%">:</td>
          
            <td colspan="4" style="text-align:left;">{{date("d-F-Y", strtotime($item->tgl))}}</td>
            
            
        </tr>
        <tr>
            <td colspan="3" style="text-align:left">Nama Pengirim</td>
            <td style="text-align:center">:</td>
            <td colspan="4" style="text-align:left">{{$item->pengirim}}</td>
        </tr>
        <tr>
            <td colspan="3" style="text-align:left">Alamat Pengirim</td>
            <td style="text-align:center">:</td>
            <td colspan="4" style="text-align:left">{{$item->alamat}}</td>
        </tr>
        <tr>
            <td colspan="3" style="text-align:left">Nomor Surat</td>
            <td style="text-align:center">:</td>
            <td colspan="4" style="text-align:left">{{$item->no_surat}}</td>
        </tr>
        <tr>
            <td colspan="3" style="text-align:left">Perihal</td>
            <td style="text-align:center">:</td>
            <td colspan="4" style="text-align:left">{{$item->perihal}}</td>
        </tr>
        @endforeach
        <tr>
            <td colspan="7" style="text-align:left;height:10px"></td>
            
        </tr>
        <tr>
            <td colspan="7" style="text-align:left"><strong> PERJALANAN SURAT</strong></td>
        </tr>
        <tr>
            <td colspan="3" style="text-align:left">Disposisi dari</td>
            <td style="text-align:center">:</td>
            @foreach ($pengirim as $item)
            <td colspan="4" style="text-align:left">{{$item->jabatan}}</td>
             @endforeach
        </tr>
        @foreach ($surat as $item2 )
        <tr>
            <td colspan="3" style="text-align:left">Disampaikan Kepada</td>
            <td style="text-align:center">:</td>
            <td colspan="4" style="text-align:left">{{$item2->jabatan}}</td>
        </tr>
        <tr>
            <td colspan="3" style="text-align:left">Isi Disposisi</td>
            <td style="text-align:center">:</td>
            <td colspan="4" style="text-align:left">{{$item2->isi}}</td>
        </tr>
        @endforeach
        <tr>
            <td colspan="7" style="text-align:left;height:20px"></td>   
        </tr>
    </table>
    <table style="width:100%" class="diteruskan">
        <tr class="tr">
            <td class="td" style="text-align:center;width:2%"><strong>No</strong></td>
            <td class="td"  style="text-align:center;width:10%"><strong>Diteruskan Kepada</strong></td>
            <td  class="td"  style="text-align:center;width:20%"><strong>Isi Disposisi</strong></td>
        </tr>
        {{$i = 1;}}
        @foreach ( $forward as $item )
            <tr class="tr">
                <td class="td"  style="text-align:center">{{$i++}}</td>
                <td  class="td"  style="text-align:left">{{$item->jabatan}}</td>
                <td class="td"  style="text-align:left">{{$item->isi_forward}}</td>
            </tr>
        @endforeach
        
    </table>
    
	

</body>
</html>